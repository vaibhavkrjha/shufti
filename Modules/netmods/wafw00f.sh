...
if [[ -d '/opt/shufti/results/w00f' ]]; then
	sleep 1
else
	mkdir /opt/shufti/results/w00f
fi
echo -e "${ORNG}"
...
sudo wafw00f ${WAF_T} ${XREDIR} ${CPROXY} ${TARGET} >> /opt/shufti/results/w00f/${TARGET}.txt
echo -e "${W}Results saved to /opt/shufti/results/w00f${NC}"
sleep 5
cd /opt/shufti
shufti -m
