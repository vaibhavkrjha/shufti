...
if [[ -d /opt/shufti/results/ASM ]]; then
    echo ""
else
    sudo mkdir /opt/shufti/results/ASM
fi
echo -e "${RED}"
	...
	SC='-sc'
		...
		if [[ ${STLTH1} == y ]]; then
			STLTH='--stealth'
				...
				echo "Please wait...."
				sudo python3 asm.py -t ${TARGET} -ln -w resources/${LIST} -o /opt/shufti/results/ASM/${DOC} -f ${FORMAT} ${SC} ${STLTH}
