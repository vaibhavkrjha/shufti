...
if [[ -d '/opt/shufti/results/Sherlock' ]]; then
    sleep 2
else
    mkdir /opt/shufti/results/Sherlock
fi
cd /opt/sherlock
echo -e "${ORNG}"
...
read USERS
echo -e "${W}===========================================================================${NC}"
sudo python3 serlock.py --verbose ${RANK} -fo /opt/shufti/results/Sherlock ${TOR} --csv --timeout ${TOUT} ${PFOUND} '${USER}'
echo -e "${YLW}Results have been saved to /opt/shufti/results/Sherlock"
echo -e "${W}===========================================================================${NC}"
cd /opt/shufti
shufti -m
